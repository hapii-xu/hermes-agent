...
slide.addText("Title", {
  ...
  margin: 0  // 当需要文字与形状或图标等元素对齐时设为 0
});
```

**提示：** 文本框默认有内部边距。当需要文字与同 x 位置的形状、线条或图标精确对齐时，把 `margin` 设为 `0`。
...
// ❌ 错误：绝不要用 unicode 项目符号
slide.addText("• First item", { ... });  // 会生成双重项目符号
...
// 圆角矩形（rectRadius 仅对 ROUNDED_RECTANGLE 生效，对 RECTANGLE 无效）
// ⚠️ 不要与矩形强调条叠加使用——它们盖不住圆角。改用 RECTANGLE。
...
| `color` | string | 6 位十六进制（如 `"000000"`） | 不要带 `#` 前缀，不要用 8 位十六进制——见「常见陷阱」 |
| `blur` | number | 0-100 pt | |
| `offset` | number | 0-200 pt | **必须非负**——负值会损坏文件 |
| `angle` | number | 0-359 度 | 阴影投射方向（135 = 右下，270 = 向上） |
| `opacity` | number | 0.0-1.0 | 透明度用这个，绝不要编码进颜色字符串 |

要让阴影向上投射（例如在页脚条上），用 `angle: 270` 加正偏移——**不要**用负偏移。

**注意**：原生不支持渐变填充。改用渐变图片作为背景。
...
用 react-icons 生成 SVG 图标，再栅格化为 PNG 以获得通用兼容性。
...
**注意**：图标用 256 或更高的尺寸以保证清晰。size 参数控制的是栅格化分辨率，而非幻灯片上的显示大小（显示大小由 `w` 和 `h` 的英寸值设定）。
...
⚠️ 这些问题会导致文件损坏、视觉 bug 或输出破损，务必避免。
...
2. **绝不要把不透明度编码进十六进制颜色字符串**——8 位颜色（如 `"00000020"`）会损坏文件。请改用 `opacity` 属性。
   ```javascript
   shadow: { type: "outer", blur: 6, offset: 2, color: "00000020" }          // ❌ 损坏文件
   shadow: { type: "outer", blur: 6, offset: 2, color: "000000", opacity: 0.12 }  // ✅ 正确
   ```

3. **用 `bullet: true`**——绝不使用「•」之类的 unicode 符号（会生成双重项目符号）

4. **数组项之间用 `breakLine: true`**——否则文本会挤在一起

5. **项目符号避免使用 `lineSpacing`**——会产生过大间隙，改用 `paraSpaceAfter`

6. **每份演示文稿都要新建实例**——不要复用 `pptxgen()` 对象

7. **绝不跨调用复用 option 对象**——PptxGenJS 会就地修改对象（例如把阴影值转成 EMU）。把同一个对象用于多次调用会损坏第二个形状。
   ```javascript
   const shadow = { type: "outer", blur: 6, offset: 2, color: "000000", opacity: 0.15 };
   slide.addShape(pres.shapes.RECTANGLE, { shadow, ... });  // ❌ 第二次调用拿到的是已转换的值
...
8. **不要把 `ROUNDED_RECTANGLE` 与强调边框搭配**——矩形叠加条盖不住圆角。改用 `RECTANGLE`。
   ```javascript
   // ❌ 错误：强调条盖不住圆角
   slide.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 1, y: 1, w: 3, h: 1.5, fill: { color: "FFFFFF" } });
   slide.addShape(pres.shapes.RECTANGLE, { x: 1, y: 1, w: 0.08, h: 1.5, fill: { color: "0891B2" } });

   // ✅ 正确：用 RECTANGLE 保持干净对齐
