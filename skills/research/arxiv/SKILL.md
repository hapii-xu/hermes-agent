---
name: arxiv
description: "按关键词、作者、分类或 ID 搜索 arXiv 论文。"
version: 1.0.0
author: Hermes Agent
license: MIT
...
通过 arXiv 免费的 REST API 搜索并获取学术论文。无需 API 密钥，无需依赖——只需 curl。
...
该 API 返回 Atom XML。用 `grep`/`sed` 解析，或通过 `python3` 管道输出整洁的结果。
...
# 全文（PDF → markdown，通过 Firecrawl）
web_extract(urls=["https://arxiv.org/pdf/2402.03300"])
```

本地 PDF 处理请参见 `ocr-and-documents` skill。
...
`scripts/search_arxiv.py` 脚本负责 XML 解析并提供整洁输出：
...
arXiv 不提供引用数据或推荐。为此请使用 **Semantic Scholar API**——免费，基本使用无需密钥（1 请求/秒），返回 JSON。
...
# 按 Semantic Scholar 论文 ID 或 DOI
...
|-----|------|------|
| arXiv | 约 1 请求 / 3 秒 | 无需 |
| Semantic Scholar | 1 请求 / 秒 | 无需（带 API 密钥可达 100/秒） |

## 注意事项

- arXiv 返回 Atom XML——用辅助脚本或解析代码片段获得整洁输出
- Semantic Scholar 返回 JSON——通过 `python3 -m json.tool` 提升可读性
...
- `arxiv.org/abs/1706.03762` 总是解析到**最新**版本
- `arxiv.org/abs/1706.03762v1` 指向**特定**的不可变版本
- 生成引用时，保留你实际阅读的版本后缀以防止引用漂移（后续版本可能大幅修改内容）
- API 的 `<id>` 字段返回带版本的 URL（例如 `http://arxiv.org/abs/1706.03762v7`）

## 撤回论文

论文在提交后可能被撤回。发生这种情况时：
- `<summary>` 字段包含撤回通知（留意 "withdrawn" 或 "retracted"）
- 元数据字段可能不完整
- 在把结果当作有效论文对待之前，务必检查摘要
