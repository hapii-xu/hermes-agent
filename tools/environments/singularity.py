...
通过 --containall、--no-home、capability dropping 进行安全加固。
支持可配置的资源限制，以及通过跨会话存活的可写 overlay 目录
实现的可选文件系统持久化。
"""
...
def _find_singularity_executable() -> str:
    """定位 apptainer 或 singularity CLI 二进制。"""
...
class SingularityEnvironment(BaseEnvironment):
    """带资源限制与持久化的安全加固 Singularity/Apptainer 容器。

    每次调用都 spawn：每次 execute() 都 spawn 一个全新的
    ``apptainer exec ... bash -c`` 进程。会话快照跨调用保留环境变量。
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        """在 Singularity 实例内 spawn 一个 bash 进程。"""
